...
O sistema SHALL exibir um botão "Nova Conta Recorrente" que abre um dialog com formulário (`descricao`, `valorPadrao`, `categoria`, `diaVencimento` opcional, `competenciaInicio`, `competenciaFim` opcional), validado com zod, e SHALL chamar `POST /api/v1/contas-recorrentes` ao submeter.
...
O sistema SHALL permitir editar uma conta recorrente existente através de um dialog pré-preenchido, chamando `PUT /api/v1/contas-recorrentes/{id}` ao submeter.
